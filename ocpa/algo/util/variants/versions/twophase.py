...
#确定流程执行变体的两阶段方法：首先通过计算流程执行的字典顺序表示(lexicographical respresentation)来识别变体，然后通过一对一同构比较来进一步细化这些类别
#方法允许通过设置参数来精确计算和细化变体，通常情况下，调用这个方法会集成到OCEL类中，并且在导入OCEL时的参数设置中指定。
